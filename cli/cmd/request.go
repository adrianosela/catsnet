...
var requestCmd = &cobra.Command{
	Use:   "request",
	Short: "Request a certificate from the catsnet CA",
	Long: `Generate a private key and CSR, then request a certificate from the catsnet CA server.
...
func runRequest(cmd *cobra.Command, args []string) {
	...
	resp, err := http.Post(serverURL+"/sign", "application/json", bytes.NewReader(reqJSON))
	if err != nil {
	...
	if err := os.WriteFile(certOutPath, []byte(csrResp.Certificate), 0644); err != nil {
